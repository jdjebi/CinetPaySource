 ...
 */
@Component
public class BasicApiDispatcher {
	...
	public void setup(KafkaService kafkaService, OperatorRepository operatorRepository) {
	...
	public void startDispatch(Transaction trx, Operator operator) throws JsonProcessingException {
		...
		if(operator.getResourcesCode().equals(RX_CODE.API)) {
			...
			map.put("error", "TransactionOperatorNotSupported");
	...
	public void apiMethod(Transaction trx, Operator operator) throws JsonProcessingException {
		...
		if(operator.getApi() != null) {
			...
			if(operatorApiBalanceIsEnough(trx, operator)) {
				...
				if(operator.getApi().isIgnoreBalance() == true) {

					kafkaService.pushEvent("TRANSACTION","TransactionIgnoreApiBalance",trxRef, Event.LOG_TYPE, Event.WARNING);

					logger.warn(String.format("Trx processing balance ignore, trx will be send to api transfert service on topic %s: %s",operator.getTopicApi(), trxRef));
	...
	/**
	 * Envoie la transaction a un service d'api
	 * @throws JsonProcessingException 
	 */
	public void sendTrxToApi(Transaction trx, Operator operator) throws JsonProcessingException{
