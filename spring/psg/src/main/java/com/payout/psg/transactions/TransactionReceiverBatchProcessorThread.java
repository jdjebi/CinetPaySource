 ...
 */
public class TransactionReceiverBatchProcessorThread extends Thread {
	...
	public TransactionReceiverBatchProcessorThread(List<Transaction> trxList, TransactionProcessComponentsProxy trxComponentsProxy, List<TransactionReceiveStatus> trxReceiveStatusList, EventTransaction event, Map<String, List<Operator>> operatorMap){	    
	...
	public void run(){
		...
		for(Transaction trx: trxList)		
			if(!validateTransaction(trx)) // Transaction invalide, on passe a la suivante, son statut est automatiquement mis a jour
				continue;
			
		// Traitement
		if(!newTrxList.isEmpty())
			process(trxList);
	...
	public boolean validateTransaction(Transaction trx) {
		...
		// Emission d'un evenement de reception
		kafkaEventService.pushForEntity(event, trx);
	 ...
	 * @return
	 */
	public boolean atomicProcess(Transaction trx) {
